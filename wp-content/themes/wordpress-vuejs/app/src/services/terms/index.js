...
export default {
  async getAll (config = {}) {
    ...
    let response = await fetch(`${apiBaseUrl}${config.taxonomy}?${body}`)
  ...
  async getById (id, config = {}) {
    ...
    return this.privateGet(`${apiBaseUrl}${config.taxonomy}/${id}${config.embed ? '?_embed' : ''}`, config)
  },

  async getBySlug (slug, config = {}) {
    config = Object.assign({}, getConfig, config)
    return this.privateGet(`${apiBaseUrl}${config.taxonomy}?slug=${slug}${config.embed ? '&_embed' : ''}`, config)
  },
